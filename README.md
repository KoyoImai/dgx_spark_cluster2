...
・[QSFPケーブル対応スイッチ（4台までQSFPで接続可能）](https://mikrotik.com/product/crs812_ddq)


## 構成（予定）
・管理者node : DGX Spark 08 \
・計算用node : DGX Spark 15 ~ 18 \
・管理者nodeと計算用nodeをRJ45 Ethernet スイッチ経由で接続 or 管理者nodeと計算用nodeをQSFPスイッチで接続（適宜切り替え）\
・速度比較のためにDGX SparkをQSFPケーブルで直接接続する場合もあり \
...
**[ステップ006：QSFPの4台接続（QSFPスイッチ）-失敗](https://github.com/KoyoImai/dgx_spark_cluster2/tree/main/Step006)**
