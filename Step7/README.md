...
- 4nodeで**3.86倍**のスループット向上（理想値4倍に対してスケーリング効率97%）
- 2node-RJ45と2node-QSFPの差がほぼない理由：LoRAの勾配データ量が約5MBと小さく、RJ45（1Gbps）でも通信がボトルネックにならないため
...
### 2node-QSFP
```
cat > /home4cluster/lora_train/run_alpaca_2node_qsfp.sh << 'EOF'
#!/bin/bash
...
EOF
chmod +x /home4cluster/lora_train/run_alpaca_2node_qsfp.sh
```
```
bash /home4cluster/lora_train/run_alpaca_2node_qsfp.sh
```


### 4node-RJ45
```
cat > /home4cluster/lora_train/run_alpaca_4node_rj45.sh << 'EOF'
#!/bin/bash
...
EOF
chmod +x /home4cluster/lora_train/run_alpaca_4node_rj45.sh
```
```
bash /home4cluster/lora_train/run_alpaca_4node_rj45.sh
```
